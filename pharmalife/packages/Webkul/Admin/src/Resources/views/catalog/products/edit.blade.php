...
<div class="content">
    ...
    <form method="POST" action="" @submit.prevent="onSubmit" enctype="multipart/form-data">
        ...
        <div class="page-content">
            ...
            @if (count($customAttributes) && $attributeGroup->name!='Shipping')
            ...
            <accordian :title="'{{ __($attributeGroup->name) }}'" :active="{{$index == 0 ? 'true' : 'false'}}">
                <div slot="body">
                    ...
                    <div class="control-group {{ $attribute->type }}" @if ($attribute->type == 'multiselect') :class="[errors.has('{{ $attribute->code }}[]') ? 'has-error' : '']"
                        @else :class="[errors.has('{{ $attribute->code }}') ? 'has-error' : '']" @endif>

                        <label for="{{ $attribute->code }}" {{ $attribute->is_required ? 'class=required' : '' }}>
                           
            
                        {{ ($att_translate->get_label_name($attribute->id,app()->getLocale())) ? $att_translate->get_label_name($attribute->id,app()->getLocale()):  $attribute->admin_name   }}
                        ...
                        <span class="control-error" @if ($attribute->type == 'multiselect') v-if="errors.has('{{ $attribute->code }}[]')"
