...
//dd($cart);
//exit;
        return view('shop::checkout.onepage2',compact('cart'));
    }
    ...
    */
    ...
    /**
     * save full order function
     *
     * @param CustomerAddressForm $request
     * @return void
     */
    public function saveFullOrder(){
      //  CustomerAddressForm $request
            $_data = request()->all();
           // return response()->json($data);
           unset($_data['_token']);
             $data['billing']=$_data;
             $data['billing']['address1']=[$_data['address1']];
            ...
            $saved=Cart::saveCustomerAddress($data);
            if (Cart::hasError() || ! $saved) {
                return redirect()->route('shop.checkout.cart.index');
